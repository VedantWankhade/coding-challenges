...
/**
 <p>
 Problem - https://www.geeksforgeeks.org/problems/the-pattern-matcher-java/1
 </p>
...
public class PatternMatcher {
    public static void main(String[] args) {
        ...
        follPatt("xyxy");
        follPatt("xy");
        follPatt("xxxyyyxxyyxy");
        follPatt("xxxyyyyxxyyxyx");
    }

    static void follPatt(String s) {
        if (s.length() % 2 != 0 || s.charAt(0) == 'y') {
            System.out.println(0);
            return;
        }
        int  count = 0;
        boolean wannaCompare = false;
        for (int i = 0; i < s.length(); i++) {
            if (s.charAt(i) == 'x') {
                if (wannaCompare && count != 0) {
                    break;
                } else {
                    count++;
                    wannaCompare = false;
